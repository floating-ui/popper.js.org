...
Your tooltip is now a "floating" element — it only takes up as much size as it
...
position(button, tooltip).then(({ x, y }) => {
  Object.assign(tooltip.style, {
    left: `${x}px`,
    top: `${y}px`,
  });
...
Above, we call `PopperDOM.position()` with the button and tooltip elements as
arguments. It returns a `Promise`, so we use the `.then()` method which passes
in the calculated `x` and `y` coordinates for us, which we use to assign `left`
and `top` styles to the tooltip.
...
```js {1,5}
const { position, flip, shift } = PopperDOM;
...
```js {1,5}
const { position, flip, shift, offset } = PopperDOM;
...
const { position, flip, shift, offset, arrow } = PopperDOM;
...
This contains an `arrow` object, referring to the name of the `Popper.arrow()`
modifier we used:
...
position(button, tooltip, {
  ...
  Object.assign(arrow.style, {
    ...
    [{
      ...
      bottom: 'top',
      left: 'right',
    }[placement.split('-')[0]]]: `${modifiersData.arrow.length / 2}px`,
  });
});
```
