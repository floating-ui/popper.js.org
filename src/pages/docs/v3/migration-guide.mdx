...
// Set initial styles to ensure `computePosition()` reads the correct
// dimensions.
applyStyles();

// `computePosition` is async, returning a Promise.
computePosition(reference, popper).then(applyStyles);
```
...
`computePosition()` now only positions the popper based on the `placement`
option that was passed in. It does not do anything else, meaning there are no
modifiers by default. This makes it much more **predictable** to use.

Likely, you'll want to add the `flip` and `shift` modifiers in:

```js
import { computePosition, flip, shift } from '@popperjs/dom';
...
This was an option in Popper 2's `preventOverflow` modifier. This is now a
tree-shakeable function called `limitShift`, which you can pass as the `limiter`
option to `shift()`.

## 5. Event listeners are up to you

`computePosition()` is a single pure function call, it does not do any "magic"
like `createPopper()` did like adding listeners to update the popper.
...
// Add listeners on window `scroll` and `resize`
...
You now fully control the ordering of the modifiers. There is no concept of
`phase` or `requires` like in Popper 2.
...
```js
modifiers: [offset({ mainAxis: 10, crossAxis: 5 })];
```

Which is more consistent with other modifier APIs.

## 9. `altAxis` is now `crossAxis`

Everywhere `altAxis` was used, rename to `crossAxis`.

## 10. `auto` placements are their own modifier

Popper 2:

```js
createPopper(reference, popper, {
  // required the `flip` modifier
  placement: 'auto',
});
```
...
The behavior is also different, but should be more accurate/smarter.
