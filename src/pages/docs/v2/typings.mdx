...
after the `// @flow` header has been removed.

### Modifiers Type Checking

`createPopper`, by default, works in a loose-type mode, that means the modifiers
...
type Modifiers = StrictModifiers | $Shape<CustomModifier>;
